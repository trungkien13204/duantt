...
function toNonNullObservable(subject) {
  return subject.asObservable().pipe((0, import_operators.filter)((value) => value !== null));
}
var RecaptchaLoaderService = class _RecaptchaLoaderService {
  ...
  constructor(platformId, language, baseUrl, nonce, v3SiteKey, options) {
    ...
    this.ready = subject ? toNonNullObservable(subject) : (0, import_rxjs.of)();
  }
  /** @internal */
  init() {
    ...
    const subject = new import_rxjs.BehaviorSubject(null);
...
var ReCaptchaV3Service = class _ReCaptchaV3Service {
  ...
  get onExecute() {
    if (!this.onExecuteSubject) {
      this.onExecuteSubject = new import_rxjs.Subject();
  ...
  get onExecuteError() {
    if (!this.onExecuteErrorSubject) {
      this.onExecuteErrorSubject = new import_rxjs.Subject();
